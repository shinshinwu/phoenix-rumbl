defmodule RumblWeb.VideoControllerTest do
  ...
  describe "with a logged-in user" do
    ...
    test "does not create video and renders errors when invalid", %{conn: conn} do
      ...
      conn = post conn, Routes.video_path(conn, :create), video: @invalid_attrs assert html_response(conn, 200) =~ "check the errors"
      assert video_count() == count_before
    end

    test "authorizes actions against access by other users", %{conn: conn} do owner = user_fixture(username: "owner")
